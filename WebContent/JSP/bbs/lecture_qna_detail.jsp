...
<body>
	<div class="container">
		...
		<div class="title">
			<h1>${lecture.name } Q&A</h1>
			<a href="mainPage"><img class="logo_img" src="image/logo.PNG"
				alt="로고"></a>
		</div>
		<hr />
		<section>
			<form method="post" action="lecture_qna_return_detail ">
				<input type='hidden' name='qnano' value="${lq.qnano}">

				<table>
					<c:if test="${isWriter eq 0}">
						<tr>
							<td class="td1"><h3>공개 여부</h3></td>
							<c:if test="${lq.open eq 0}">
					...
					<c:if test="${isWriter eq 1}">
						...
						<tr>
							<td class="td1"><h3>내용</h3></td>
							<td><textarea name='content'
									style="width: 400px; height: 150px;" placeholder='내용을 입력해주세요'>${lq.content}</textarea>
							</td>
						</tr>
					</c:if>


					<c:if test="${empty lq.answer }">

						<c:if test="${isInstructor eq 1}">
							<tr>
								<td>
									<h2>답변</h2>
								</td>
								<td><textarea name='answer'
										style="width: 400px; height: 150px;" placeholder='답변을 입력해주세요'>${lq.answer}</textarea>
								</td>
							</tr>
						</c:if>
					</c:if>
					...
					<c:if test="${!empty lq.answer }">
						<tr>
							...
							<c:if test="${isInstructor eq 0}">
								<td><textarea name='answer' cols='40' rows='5' readonly>${lq.answer}</textarea>
								</td>
							</c:if>
							<c:if test="${isInstructor eq 1}">
								<td><textarea name='answer' cols='40' rows='5'
										placeholder='답변을 입력해주세요'>${lq.answer}</textarea></td>
							</c:if>
						</tr>
					</c:if>
